...
class _LeaderboardScreenState extends State<LeaderboardScreen> {
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          if (topThree.length < 2) const SizedBox(width: 70), // Placeholder
          ...
          if (topThree.length < 3) const SizedBox(width: 70), // Placeholder
        ],
      ),
    );
  }
  ...
  Widget _topUser(String name, String rank, String points, String? avatar, double size, {bool isFirst = false}) {
    return Column(
      children: [
        if (isFirst) const Icon(Icons.emoji_events_rounded, color: Colors.amber, size: 24),
        CircleAvatar(
          radius: size / 2, 
        ...
        Text('$points pts', style: const TextStyle(color: AppColors.primary, fontSize: 12, fontWeight: FontWeight.bold)),
        Container(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Container(
          ...
          child: Row(
            children: [
              Text('#$rank', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.grey, fontSize: 16)),
              ...
              const SizedBox(width: 12),
              Text(isMe ? 'Moi (${entry.pseudo})' : entry.pseudo, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
              const Spacer(),
              Text('${entry.points} pts', style: const TextStyle(fontWeight: FontWeight.bold, color: AppColors.onBackground)),
            ],
          ),
        );
      },
